#!/bin/sh
# Checks upstream tailscale for a new release and fires OS5 alert 1501 if found.
# Added to root crontab by install.sh; removed by remove.sh.
# Can also be run manually: sh /path/to/tailscale/check_update.sh

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
APP_NAME="tailscale"
UPSTREAM_API="https://api.github.com/repos/tailscale/tailscale/releases/latest"
